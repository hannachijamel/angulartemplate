...
import { ListecompteurService } from './listecompteur.service';
...
@Component({
  selector: 'app-listecompteur',
  templateUrl: './listecompteur.component.html',
  styleUrls: ['./listecompteur.component.css']
})
export class ListecompteurComponent implements OnInit {
  ...
  typepreventives!:any;
    ...
    cols!: any[];
    constructor(private listservice:ListecompteurService,
      private equipementservice: EquipementService,
       private messageService: MessageService, private confirmationService: ConfirmationService,
       private fb:FormBuilder,private typepreventiveservice:TypeprevService) {
         
      this.listservice.getAllPreventiveParCompteur().then(data =>{
...
onDelete(id:any) {
  this.confirmationService.confirm({
    ...
    accept: () => {
      this.listservice.deletePreventiveParCompteur(id).subscribe(data=>{
...
if(this.typeDialog=='add')  {   this.listservice.savePreventiveParCompteur(this.prevenitiveparcompteurFormGroupe.value).subscribe(data=>{
...
else if(this.typeDialog=='edit') {this.listservice.updatePreventiveParecompteur(this.prevenitiveparcompteurFormGroupe!.value,this.selectedPrevenitiveparcompteur.id).subscribe(data=>{
  if(data.status==='success' && data.data==1)
  this.listservice.getAllPreventiveParCompteur().then(data=>{
      this.prevenitiveparcompteurs=data
